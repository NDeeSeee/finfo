...
MY OWN THOUGHTS:
    ...
    IF FILE SIZE CHANGING FAST - SHOW A BAR WITH A PROGRESS OF DOWNLOADING / UPLOADING (for example, if you are downloading a file, and the size is changing fast, you should see a bar with a progress of downloading)
        \\ same for cp/mv/rm/..... ESPECIALLY FOR rm
        \\ for rm it should be also a separate plugin to add when I just run rm -rf .
        \\ so it have to automatically check process which running with that file to print out either it moving, copying, downloading so on. 
    ...
    ADD INTERNAL CONVERTER OF ENCODING / 
    ADD (as a separate tool) PARSE .zshrc and show all the aliases, functions, etc.
    ADD (as a separate tool) MASTER OF PACKAGES (like pip, conda, mamba, brew, npm, R, yarn etc)
    ADD NOTE FEATURE, TODO, QUICKACTIONS
